...
//    }
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) {
        if (msg instanceof HttpRequest) {
            ...
            if (request.headers() != null && "true".equals(request.headers().get("forward"))) {
                // 能处理大量请求的原因是因为调用的时候，使用了非阻塞的方式
                NettyHttpClient.send(ctx);
                responseData.setLength(0);
                responseData.append("forward");
                return;
            }

//            responseData.setLength(0);
//            responseData.append(RequestUtils.formatParams(request));
            ByteBuf byteBuf = Unpooled.copiedBuffer("direct", CharsetUtil.UTF_8);
